...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "BopuNetwork") type = BopuNetwork;
    if(strType == "BopuNetworkVariable") type = BopuNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == BopuNetwork) s = "BopuNetwork";
    if(type == BopuNetworkVariable) s = "BopuNetworkVariable";
