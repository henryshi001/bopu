...
*Setup instructions for a Gitian build of Bopu Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bopu
...
- [Building Bopu Core](#building-bopu-core)
...
Clone the git repositories for Bopu Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bopucoin/bopu
```
...
in the Bopu Core repository under 'Fetch and build inputs' to install sources which require
...
To build Bopu Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bopu Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bopu/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bopucoin/bopu
...
For example:
```bash
URL=https://github.com/crowning-/bopu.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bopu=${COMMIT} --url bopu=${URL} ../bopu/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bopu=${COMMIT} --url bopu=${URL} ../bopu/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bopu=${COMMIT} --url bopu=${URL} ../bopu/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bopu git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bopu/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bopu.git
SIGPATH=/some/root/path/bopu-detached-sigs.git

./bin/gbuild --url bopu=${BTCPATH},signature=${SIGPATH} ../bopu/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bopu-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bopu-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bopu-osx-build.assert
```
...
[bopu/gitian.sigs](https://github.com/bopucoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
